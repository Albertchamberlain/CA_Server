...
func (ca *CertificateAuthority) load() {
	//如果没有配置根证书，需要自签一个，只有有了证书才能够给别的用户请求签发证书
...
/*
CA 做一个自签名证书，作为自己的根证书，当配置没有在cert\rootCA下提供根证书和私钥时，需要自己做一个
*/
...
/*
用根证书签署一个证书签发请求CSR。CSR是以自定义的Struct表达的
*/
func (ca *CertificateAuthority) SignX509(csr *CertificateSigningRequest) (*Certificate, error) {
	...
	cx509CSR := csr.toCX509CSR(csrPrivateKey) //将从HTTP接收的CSR转换为CX509CSR
	...
	err = saveToPEM(buf, clientCAFolder, fileNamePrefix+".crt", "CERTIFICATE") //将生成的证书存储到指定目录
	if err != nil {
		log.Print("persistent generated certificate file fail")
		return nil, err
	}
	err = saveToPEM(cx509.MarshalPKCS1PrivateKey(csrPrivateKey), clientCAFolder, fileNamePrefix+".key", "PRIVATE KEY") //将生成的私钥存储到指定目录
	if err != nil {
